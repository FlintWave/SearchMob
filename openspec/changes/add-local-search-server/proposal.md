...
SearchMob's whole value is being a search service you can actually *use* from the phone — not just
a Compose screen, but a localhost endpoint that the system browser and other on-device apps can
...
- New code (packages established in scaffold): `server/` — Ktor server bootstrap, route handlers
